...
const Create = () => {
    const { addItem, items, timeToString } = useContext(DataContext) as CalendarContextType;
    ...
    const createNote = useCallback(() => {
        const doesNoteExist = items[selectedDate]?.find((item: AgendaEntry) => item.name === note);
        ...
        if(items[selectedDate]){
            addItem({
                [selectedDate] : [...items[selectedDate], {name: note, height: 50, day: selectedDate}]
            })
        } else {
            addItem({
    ...
    return (
        <View style={styles.container}>
            ...
            />
            <TextInput value={note} onChangeText={setNote} style={styles.input} multiline={true} numberOfLines={4} />
